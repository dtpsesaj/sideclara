...
def serialize_ingreso(declaracion, tipo):
    ...
    if not ingresos:
        ...
        for ingreso in ingresos:
            if not tipo:
                ingreso_anio_anterior_fechas["fechaIngreso"] = ingreso.fecha_ingreso if ingreso.fecha_ingreso else "2020-01-01"
                ingreso_anio_anterior_fechas["fechaConclusion"] = ingreso.fecha_conclusion if ingreso.fecha_conclusion else "2020-01-01"
...
def serialize_datos_empleado_cargo_comision(encargo,declaracion):
    ...
    if puesto:
        if not "SIN_PUESTO" in puesto:
            ...
            serialized["SIN_PUESTO"] = puesto["SIN_PUESTO"]
...
def serialize_empleo_cargo_comision(declaracion, encargo):
    ...
    try:
        ...
        puesto_id = historico.id_puesto
        puesto_codigo = historico.nivel
        area = "{}-{}".format(historico.codigo_area,historico.txt_area)
        puestoDic = {"puesto": puesto, "puesto_codigo": puesto_codigo, "area": area, "puesto_id": puesto_id}
    except Exception as e:
        puestoDic = {"SIN_PUESTO": {"DECLARANTE": declaracion.info_personal_fija.pk, "DECLARACION": declaracion.pk}}
