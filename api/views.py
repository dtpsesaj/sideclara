...
from declaracion.models.catalogos import CatPuestos, CatEntesPublicos
from django.contrib.auth.models import User
...
def declaracionesJsonView(request):
    ...
    p = Popen(['python3', './toJSONScript.py'])
...
def consultaEstatusTaskJSONDeclaracion(request):
    ...
    if estatus == 100:
        path_static = settings.STATIC_URL + 'declaraciones'
        response["path"] = path_static + '/declaraciones.json'
...
class ActualizarUsuarioInfoPerFija(ProtectedResourceView):
    def post(self, request, *args, **kwargs):
        json_data = json.loads(request.body)
        print(json_data)
        ...
        try:
            ...
            usuario = User.objects.get(username=json_data.get('rfc'))
            usuario.is_active = True
            usuario.first_name = json_data.get('nombres')
            usuario.last_name = json_data.get('apellido1')+' '+json_data.get('apellido2')
            usuario.email = json_data.get('email')
            usuario.set_password(json_data.get('password'))
            usuario.save()
            puesto_obj = CatPuestos.objects.get(pk = json_data.get('puesto'))
            ente_publico_obj = CatEntesPublicos.objects.get(pk = json_data.get('ente_publico'))
            info_per_fija = InfoPersonalFija.objects.create(
                nombres = json_data.get('nombres'),
                apellido1 = json_data.get('apellido1'),
                apellido2 = json_data.get('apellido2'),
                puesto = puesto_obj.puesto,
                cat_puestos = puesto_obj,
                rfc = json_data.get('rfc'),
                telefono = json_data.get('telefono'),
                usuario = usuario,
                cat_ente_publico = ente_publico_obj,
                nombre_ente_publico = ente_publico_obj.ente_publico,
                fecha_nacimiento = json_data.get('fecha_nacimiento'),
