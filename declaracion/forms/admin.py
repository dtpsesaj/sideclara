...
class RegistroUsuarioDeclaranteEdicionForm(forms.Form):
    ...
    def clean_rfc(self):
        ...
        if 'id' in self.cleaned_data:  # Verificar si se está editando un usuario existente
            user_id = self.cleaned_data['id']
            user = User.objects.get(pk=user_id)
            if user.username != rfc:  # Validar el RFC solo si ha cambiado
                if User.objects.filter(username=rfc).exists():
                    raise ValidationError("Este RFC ya está registrado.")
