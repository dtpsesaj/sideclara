...
def obtener_token():
    # Configura los datos de tu aplicación cliente
    data = {
        'grant_type': 'password',
    ...
    print(data)
    response = requests.post(settings.LOGIN_URL+'/send_token/', data=data)
...
def task_crear_pdf(tipo,key_process,extra_param):
    ...
    p = Popen(['python3', './toPDFScript.py',tipo, parametro_task, extra_param])
