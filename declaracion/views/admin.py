...
from .utils import (set_declaracion_extendida_simplificada, task_crear_pdf, task_obtener_estatus,
                                    obtener_pdf_existente, task_eliminar_background, obtener_token)
    ...
    if request.user.is_staff:
        ...
        for res in result:
            ...
            data = {
                ...
                'avance':res.avance,
                'extendida': "Sí", #'No' if int(nivel) <= int(settings.LIMIT_DEC_SIMP) else 'Sí',
    ...
    def get(self, request, *args, **kwargs):
        ...
        if editar_id:
            ...
            if tipo_registro == 'registrado' or tipo_registro == "registrado_sindec":             
                ...
                area, puesto, nivel, estatus, email = data_usuario.cat_puestos.cat_areas, data_usuario.cat_puestos, data_usuario.cat_puestos.codigo, data_usuario.usuario.is_active, data_usuario.usuario.email
                PUESTO=puesto.pk                
            else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if editar_id:
            if tipo_registro == 'registrado' or tipo_registro == "registrado_sindec" or tipo_registro=="todos":
                ...
                try:
                    historico = HistoricoAreasPuestos.objects.filter(info_personal_fija=info_personal_fija_data).last()
                    if request_post.get("estatus") == "True":
                        historico.fecha_fin = None
                    else:
                        historico.fecha_fin = date.today()
                    historico.save()
                except Exception as e:
                    print("Error al actualizar ultimo dato de historico: ", e)
            ...
            if registro.is_valid():
                data = {
                    ...
                    'appname':settings.LOGIN_APP_NAME
                }
                print(data)
                token = obtener_token()
                if not token:
                    return {'error': 'Error al obtener token de acceso.'}
                headers = {
                    'Authorization': f'Bearer {token}',
                    'Content-Type': 'application/json'
                }
                response = requests.post(settings.LOGIN_URL+'/registro_usuario/', json=data, headers=headers)
...
class DescargarReportesView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if tipo_reporte:
            ...
            usernamePDF = usuario.username[0:5] + str(usuario.pk)
            print("usernamePDF---->", usernamePDF)
            response['Content-Disposition'] = "inline; filename={}_{}.pdf".format(tipo_reporte,usernamePDF)
...
def consultaEstatusReporteExcel(request):
    estatus = task_obtener_estatus("excel",1,"excel")
    response = {"estatus_proceso": estatus}
...
def eliminarProcesoExcelReporte(request):
    return JsonResponse(task_eliminar_background("excel", request))
        ...
        for res in result:
            ...
            if res.cat_estatus.pk == 4:
                ...
                try:
                    ...
                    if encargos.count() > 0:
                        encargo = encargos.first()
                        puestoS = serialize_empleo_cargo_comision(res, encargo)
                        nivel = puestoS["puesto_codigo"]
                except ObjectDoesNotExist:
                    #Se toma el nivel actual del declarante en el puesto de infoFija
                    nivel = res.info_personal_fija.cat_puestos.codigo
            else:
            ...
            data = {
                'Folio':str(res.folio),
                'Tipo declaracion':res.cat_tipos_declaracion.codigo,
                'Avance':res.avance,
                'Estatus':res.cat_estatus.estatus_declaracion,
                'Extendida': 'No' if int(nivel) <= int(settings.LIMIT_DEC_SIMP) else 'Sí',
                'Extemporanea': 'Si' if res.extemporanea else 'No',
                'Simplificada': 'Si' if res.info_personal_fija.cat_puestos.codigo > settings.LIMIT_DEC_SIMP else 'No',
                'Fecha declaracion':res.fecha_declaracion.strftime('%d-%m-%Y') if res.fecha_declaracion else 'N/A',
                'Fecha recepcion':res.fecha_recepcion.strftime('%d-%m-%Y') if res.fecha_recepcion else 'N/A',
                'Datos publicos':'Si' if res.datos_publicos else 'No',
                'Nombres':res.info_personal_fija.nombres,
                'Apellido Paterno': res.info_personal_fija.apellido1,
                'Apellido Materno': res.info_personal_fija.apellido2,
                'RFC': res.info_personal_fija.rfc,
                'Area': puestoS["area"] if "area" in puestoS else "REVISAR",
                'Puesto': puestoS["puesto"] if "puesto" in puestoS else "REVISAR",
                'Nivel': puestoS["puesto_codigo"] if "puesto_codigo" in puestoS else "REVISAR",
                'Archivo':archivo
            }
...
def descargarReportesExcel(request):
    if settings.DEVELOP:
        host = '127.0.0.1'
    else:
        host = '192.168.156.3'
    ...
    redis_cnx.hmset("excel", {1:0})
    declaraciones = consultaDeclaraciones(request)
    wb = Workbook()
    sheet = wb.active

    redis_cnx.hmset("excel", {1:20})
    #Contenido del excel
    if len(declaraciones["declaraciones"]) > 0:
        headers = list(declaraciones["declaraciones"][0].keys())
        maxColumns = string.ascii_uppercase[:len(headers)]
        headerTupla = tuple(headers)
        sheet.append(headerTupla)
            
        for cell in sheet[1]:  # Iterar sobre todas las celdas de la primera fila
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = PatternFill(start_color="808080", end_color="808080", fill_type="solid")  # Sombreado
        
        redis_cnx.hmset("excel", {1:40})
        sIndex = 2
        for index,declaracion in enumerate(declaraciones["declaraciones"]):
            for index,letter in enumerate(maxColumns):
                columnsI = index+1

                if columnsI == len(headerTupla) and declaracion[headers[index]]:
                    nombreArchivo = declaracion[headers[index]].split("/")  
                    rutaMedia = settings.MEDIA_ROOT.replace("media", "")
                    sheet.cell(row=sIndex, column=columnsI).hyperlink = rutaMedia+declaracion[headers[index]]
                    sheet.cell(row=sIndex, column=columnsI).value = nombreArchivo[-1]
                    sheet.cell(row=sIndex, column=columnsI).style = "Hyperlink"
                else:
                    sheet.cell(row=sIndex, column=columnsI).value = declaracion[headers[index]]
                
                sheet.border = Border(right=Side(style='thin'))
            sIndex+=1
        redis_cnx.hmset("excel", {1:60})
        # Ajustar el ancho de las columnas al contenido
        for column in range(1, len(headers) + 1):
            column_letter = get_column_letter(column)
            sheet.column_dimensions[column_letter].auto_size = True

        redis_cnx.hmset("excel", {0:80})
        nombre_archivo = "reportedeclaraciones.xlsx"
        buffer = io.BytesIO() 
        wb.save(buffer)
        buffer.seek(0)
        ...
        redis_cnx.hmset("excel", {1:100})
        return response
    else:
        return JsonResponse({'message': 'NO SE ENCONTRAON COINCIDENCIAS'})
