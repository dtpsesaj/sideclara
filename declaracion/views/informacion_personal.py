...
from api.serialize_functions import serialize_empleo_cargo_comision #02/05/2024


from django.views.decorators.cache import cache_page #ADD 29/02/2024
from django.utils.decorators import method_decorator #ADD 29/02/2024
...
class DeclaracionFormView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if declaracion_obj:
            ...
            #Se le asigna el email del user a info_var como el correo personal
            info_personal_var_data = model_to_dict(info_personal_var_data)
            #Se asigna el correo que proviene de info_var email_personal a una variable temporal email_institucional
            info_personal_var_data['email_institucional'] = info_personal_var_data["email_personal"] 
            info_personal_var_data["email_personal"] = usuario.email
    ...
    def post(self,request,*args,**kwargs):
        ...
        if (declaracion_is_valid and
            ...
            if 'var-email_personal' in request.POST:
                info_personal_var.email_personal=request.POST['var-email_institucional']
            ...
            usuario.email = request.POST['var-email_personal']
            usuario.save()
...
class DatosEncargoActualView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if datos_encargo_actual_data:
            ...
            #Si no tiene declaraciones previas o la declaración que inicia es INICIAL, se carga el puesto que tenga en info_fija
            #Sí no, se carga ya sea EDICION: puesto guardado en la seccion o NUEVO: Precarga el puesto de la sección de la declaración anterior
            if not previa:
                puesto = info_personal_fija_puesto.puesto
                area = info_personal_fija_puesto.cat_areas.area
                puesto_codigo = info_personal_fija_puesto.codigo
            else:
                #puesto = datos_encargo_actual_data.cat_puestos
                puesto_serialize = serialize_empleo_cargo_comision(declaracion, datos_encargo_actual_data)
                print("puesto_serialize -------------------->", puesto_serialize)
                puesto = puesto_serialize["puesto_id"]
            ...
            #Se actualiza el diccionario con los datos del puesto obtenido
            datos_encargo_actual_data.update({'cat_areas_txt': area})
            datos_encargo_actual_data.update({'nivel_encargo': puesto_codigo})
            datos_encargo_actual_data.update({'cat_puestos_txt': puesto})
            puesto = datos_encargo_actual_data["cat_puestos"]
        else:
            ...
            datos_encargo_actual_data = {
                ...
                'cat_areas_txt': info_personal_fija_puesto.cat_areas.area, 
                'nivel_encargo': info_personal_fija_puesto.codigo, 
                'cat_puestos_txt': info_personal_fija_puesto.puesto
            }
            domicilio_data = {}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (datos_encargo_actual_form.is_valid() and
            ...
            puestoFromPost = CatPuestos.objects.get(pk=int(request.POST.get('datos_encargo_actual-cat_puestos'))) 
            ...
            datos_encargo_actual.cat_puestos = puestoFromPost
        ...
        return render(request, self.template_name, {
            ...
            'observaciones_form': observaciones_form,
            'domicilio_form': domicilio_form,
            'puesto':puestoFromPost.codigo,
            'puesto_id':puestoFromPost.pk,
            'folio_declaracion': folio_declaracion,
            'avance':declaracion.avance,
            'tipo_declaracion': declaracion.cat_tipos_declaracion.codigo,
            'campos_privados': campos_configuracion(seccion,'p'),
    ...
    def get(self, request, *args, **kwargs):
        ...
        if datos_pareja_data:
            ...
            if actividad_laboral_sector_data:
                ...
                actividad_laboral_sector_data["moneda_publico"] = actividad_laboral_sector_data["moneda"] if actividad_laboral_sector_data["moneda"] else 101
                actividad_laboral_sector_data["moneda_privado"] = actividad_laboral_sector_data["moneda"] if actividad_laboral_sector_data["moneda"] else 101
            else:
                actividad_laboral_sector_data = {}
                actividad_laboral_sector_data["moneda_publico"] = 101 
                actividad_laboral_sector_data["moneda_privado"] = 101
