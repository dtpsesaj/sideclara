...
class HistoricoResourceAadmin(resources.ModelResource):
    ...
    class Meta:
        ...
        fields = ('nombre_usuario', 'fecha_inicio', 'fecha_fin', 'txt_puesto', 'nivel', 'txt_area')
    
    def dehydrate_nombre_usuario(self, obj):
        return obj.nombre_usuario()


class HistoricoResource(ImportExportModelAdmin):
    resource_class = HistoricoResourceAadmin
    list_display = ('nombre_usuario','fecha_inicio','fecha_fin','txt_puesto','nivel','txt_area')
    exclude = ['id_puesto','id_area']
    list_filter = ('txt_area', 'txt_puesto')
    search_fields = (
        'txt_puesto',
        'txt_area'
    )
