...
from declaracion.views.utils import (campos_configuracion_todos,declaracion_datos, 
                                    ...
                                    task_crear_pdf, task_obtener_estatus, obtener_token,
...
def verify_token(token):
    response = requests.post(
        settings.LOGIN_URL+'/verify_token/',
        data={'token': token,
              'client_id': settings.LOGIN_ID, 
              'client_secret': settings.LOGIN_SECRET
            }
    )
    print("verify")
    print(response)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            if token:
                ...
                if token_info['active'] and token_info['username']:
                ...
                return redirect(settings.LOGIN_URL+'login/')
...
class PersonalizacionCatalogoPuestosView(View):
    ...
    def get(self, request, pkid=None):
        if request.user.is_superuser or request.user.is_staff:
            ...
            if ("_editar" in current_url or "_eliminar" in current_url or "_agregar" in current_url) and current_url != None: 
                return redirect("personalizar_catpuestos")
    ...
    def post(self, request, pkid=None):
        if request.user.is_staff:
            ...
            if "_editar" in current_url and current_url != None: 
                ...
                token = obtener_token()
                result = self.editar(pkid, puesto, codigo, area, token)
            ...
            if "_eliminar" in current_url and current_url != None:
                ...
                token = obtener_token()
                result = self.eliminar(pkid, token)
            ...
            if "_agregar" in current_url and current_url != None:
                ...
                if value_empty == "" or value_empty is None:
                    ...
                    result = self.agregar(value, codigo, area_valuea, token)
                    if result != True:
    ...
    def editar(self, id, puesto_txt, codigo, area, token):
        ...
        response = requests.post(settings.LOGIN_URL+'/agregar_editar_puesto/', json=data, headers=headers)
        print(response)
        return True

    def eliminar(self, id, token):
        try:
            obj = CatPuestos.objects.get(pk=id)
            obj.default = True
            obj.save()
            ...
            response = requests.post(settings.LOGIN_URL+'/eliminar_puesto/', json=data, headers=headers)
            print(response.status_code)
        except Exception as e:
            return str(e) + " " + str(id)
        return True
    
    def agregar(self, value, codigo, area, token):
        try:
            ...
            return 'No se puedo agregar el puesto, validar datos' + str(e)
        return True
...
class PersonalizacionCatalogoAreasView(View):
    ...
    def get(self, request, pkid=None):
        if request.user.is_superuser or request.user.is_staff:
            current_url = resolve(request.path_info).url_name
            if ("_editar" in current_url or "_eliminar" in current_url or "_agregar" in current_url) and current_url != None: 
                return redirect("personalizar_catareas")
    ...
    def post(self, request, pkid=None):
        if request.user.is_staff:
            ...
            if "_editar" in current_url and current_url != None: 
                ...
                token = obtener_token()
                result = self.editar(pkid,area,codigo,token)
            ...
            if "_agregar" in current_url and current_url != None:
                ...
                if value == "" or value is None:
                    ...
                    token = obtener_token()
                    result = self.agregar(value_area,value_codigo,token)
                    if result != True:
                        self.context["messages"] = {result}
                    else:
                        return JsonResponse({"message": "agregado"})
    ...
    def editar(self, id, area_txt, codigo, token):
        try:
            ...
            data = {
                'origin_id': obj.pk,
                'area': area_txt if area_txt else '',
                'codigo': codigo if codigo else '',
                'appname':settings.LOGIN_APP_NAME
            }
            response = requests.post(settings.LOGIN_URL+'/agregar_editar_area/', json=data, headers=headers)
            print(response.status_code)
        except Exception as e:
            return e
        return True
    ...
    def eliminar(self, id, token):
        try:
            ...
            response = requests.post(settings.LOGIN_URL+'/eliminar_area/', json=data, headers=headers)
            print(response.status_code)
        except Exception as e:
            return str(e) + " " + str(id)
        return True
    
    def agregar(self, value_area, value_codigo, token):
        try:
